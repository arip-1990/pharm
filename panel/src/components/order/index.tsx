...
const Order: React.FC = () => {
  ...
  const {data: orders, isLoading: fetchLoading} = orderApi.useFetchOrdersQuery({pagination, order});
  ...
  return (
    <Card title='Заказы'>
      ...
      dataSource={orders?.data.map(item => ({
        ...
        status: <StatusStep statuses={item.statuses} paymentType={item.paymentType} deliveryType={item.deliveryType} />,
