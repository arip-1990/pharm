...
const Product: FC = () => {
  ...
  const { data: product, isFetching, refetch } = useGetProductQuery(
    String(slug)
  );
  const { data: categories } = useFetchCategoriesQuery();

  const getDefaultGenerator = () => {
    let crumbs = [{ href: "/products", text: "Наш ассортимент" }];

    if (categories && product?.category) {
      crumbs = [
        ...crumbs,
        ...getTreeCategories(product?.category, categories).map((item) => ({
    ...
    return [...crumbs, { href: `/products/${slug}`, text: product?.name }];
  };
  ...
  const handleShowCarousel = () =>
    product?.photos.length && setShowCarousel(true);

  return (
    <Layout
      title={
        product ? `${product?.name} - Сеть аптек 120/80` : "Сеть аптек 120/80"
      }
    ...
    >
      ...
      <Carousel
        ...
        data={product?.photos.filter((item) => item.url)}
      />
    </Layout>
  );
};
