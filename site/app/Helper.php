    ...
    public static function getStatusInfo(string $status): string
    {
        switch ($status) {
            case Status::STATUS_ACCEPTED:
                return '<span class="text-success">Принят</span>';
            case Status::STATUS_ASSEMBLED_PHARMACY:
                return '<span class="text-success">Заказ собран</span>';
            case Status::STATUS_RECEIVED_BY_CLIENT:
                return '<span class="text-success">Заказ получен</span>';
            case Status::STATUS_CAUSED_BY_DELIVERY:
                return '<span class="text-muted">Вызвана доставка</span>';
            case Status::STATUS_CANCELLED:
            case Status::STATUS_DISBANDED:
                return '<span class="text-danger">Отменен</span>';
            case Status::STATUS_FULL_REFUND:
            case Status::STATUS_RETURN_BY_COURIER:
                return '<span class="text-danger">Возврат</span>';
            default:
                return '<span class="text-muted">В обработке</span>';
        }
    }
}
